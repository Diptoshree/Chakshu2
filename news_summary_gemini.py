...
def extract_text_from_image(image):
    ...
    - **Headline**: The main news headline, usually in the largest bold font, spanning up to 3 lines with a uniform font size. Ignore subheaders, captions, dates, and newspaper organization names.
                    (Very Important)Under Headline section DO not extract ant text which has smaller font size compared to dead line. texs printed in smaller forns are subheading, location or body text. We only and only want Headline. This instruction needs to be followed strictly.
